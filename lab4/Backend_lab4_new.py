...
class crawler(object):
    ...
    def __init__(self, db_conn, url_file):
        ...
        """create these dics for pagerank"""
        self._from_doc_list = []
        self._to_doc_list = []
        self._rank_page = []
    ...
    def _visit_img(self, elem):
        ...
        img_lst = find_one(self._curr_doc_id)
        img_lst.append(img_url)
        update('documet_index', 'img', img_lst, self._curr_doc_id)
    ...
    def _add_words_to_document(self):
        for i in self._curr_words:
            if i[0] not in self._inverted_index.keys():
                self._inverted_index[i[0]] = set()
    ...
    def get_resolved_inverted_index(self):
        ...
        for k,v in self._word_id_cache.items():
            self._inverted_word_id_cache[v]=k

        for k,v in self._doc_id_cache.items():
            self._inverted_doc_id_cache[v]=k
        di=collections.OrderedDict()
        for key in self._inverted_index:
            L=[]
            for values in list(self._inverted_index[key]):
                L.append(self._inverted_doc_id_cache[values])
                self._get_inverted_index[self._inverted_word_id_cache[key]]=set(L)
